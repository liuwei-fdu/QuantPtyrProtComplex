...
pdf(
  ...
  write.csv(point, file = "PPI_select_hight_width2.csv")
  points(
    point$width,
...
#peak cv and corr
peak_corr_cv<-point
PPIs_peaks_select_both<-peak_corr_cv[peak_corr_cv$corr>0.6 & peak_corr_cv$cv>0.5, ]
 pdf(
    "PPI_select_corr_cv2.pdf",
    width = 6,
    height = 6
  )
  plot(
    peak_corr_cv$corr,peak_corr_cv$cv,
    col = "#FFA07A",
    pch = 19,
    cex = 1,
    xlab = paste("Pearson Correlation"),
    ylab = "coefficient of variation",
    main = "select by peak cv and corr"
 )

points(
    PPIs_peaks_select_both$corr,
       PPIs_peaks_select_both$cv,
    col = 1,
    bg = "#E64B35CC",
    pch = 21,
    cex = 1.5
...
legend("topright", legend=c(nrow(peak_corr_cv),nrow(PPIs_peaks_select_both)), col=c("#FFA07A", "#E64B35CC"), lty=1)
...
abline(
  ...
  write.csv(PPIs_peaks_select_both,"select_295peaks.csv")
...
```{r quantative protein complex matrix}
peak_corr_cv<-point
...
allPPI <-
  read.csv(
    "input_data/Out.rf.comb.clust.csv",
    header = F
  )
row.names(allPPI) <- paste0("PPI", row.names(allPPI))
