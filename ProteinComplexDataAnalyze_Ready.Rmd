...
dfVolcanoplot1[is.na(dfVolcanoplot1)] <- min(dfVolcanoplot1, na.rm = T) * 0.5
...
plot<-ggplot(dfVolcanoplot2, aes(x = dfVolcanoplot2$fd, y = -log10(dfVolcanoplot2$P_value))) +
  xlim(-5.5,5.5)+
  
geom_point(aes(color = ifelse(abs(fd) > 2 & P_value<0.05  ,"#87CEFA", "#00000033")),size=2)+
labs(x = "Log2 Fold Change", y = "-log10(p value)", title = "SH2_probe VS SH2")
dfVolcanoplot2$label=ifelse(abs(dfVolcanoplot2$fd) > 2  & dfVolcanoplot2$P_value < 0.05 ,row.names(dfVolcanoplot2),"")
write.csv(dfVolcanoplot2,"out/dfVolcanoplot2.csv")

p <-
  plot + geom_text_repel(
    aes(
      x = dfVolcanoplot2$fd,
      y = -log10(dfVolcanoplot2$P_value) ,
  ...
  ggsave(
    plot = p,
    filename = "SH2_SH2_Probe_volcanolplot.pdf",
    device = NULL,width = 10,height = 7)

### kinase database
  Kinase <- read_excel("input_data/Kinase.xlsx")
  dfVolcanoplot3<-dfVolcanoplot2
 dfVolcanoplot3$uniprot<-row.names(dfVolcanoplot3)
  


dfVolcanoplot4<-dfVolcanoplot3[dfVolcanoplot3$uniprot %in%  Kinase$Entry,]
  
up<-subset(dfVolcanoplot4, dfVolcanoplot4$fd> 0.5&dfVolcanoplot4$P_value< 0.05)
down<-subset(dfVolcanoplot4, dfVolcanoplot4$fd< -0.5 &dfVolcanoplot4$P_value< 0.05)
...
up <- subset(dfVolcanoplot2, dfVolcanoplot2$P_value < 0.05 & dfVolcanoplot2$fd > 2)
down <- subset(dfVolcanoplot2, dfVolcanoplot2$P_value < 0.05 & dfVolcanoplot2$fd < -2)
points(
  up$fd,
  -log10(up$P_value),
  col = 1,
...
abline(h = 1.3, v = c(-2, 2), lty = 2, lwd = 1)
dev.off()
...
```{r all ppi map to corum }
rm(list=ls())
library(dplyr)
library(readr)
library(stringr)
ppi <- read.csv("input_data/allPPIs.csv",row.names = 1) 
corum <-
  read_delim(
    "input_data/allComplexes.txt",
    delim = "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ) %>%
  as.data.frame() %>%
  mutate(corumid=paste0(ComplexID,"_",ComplexName))%>%
select(corumid,6)

corum2<-str_split_fixed(corum$`subunits(UniProt IDs)`,";",100) %>%
  as.data.frame() 
row.names(corum2)<-corum$corumid
  
df<-data.frame("ppi","corumid","ppi_subunits","ppi_subunits_in_corum","score")
names(df)<-c("ppi","corumid","ppi_subunits","ppi_subunits_in_corum","score")
df<-df[-1,]

for (i in row.names(ppi)) {
  ppis<-as.vector(as.matrix(ppi[row.names(ppi)==i,]))
  ppis<-ppis[!ppis==""]
  allppi<-apply(corum2,1,  function(x){
  tmp<-as.matrix(x)
  tmp<-tmp[!tmp==""]
  ratio_ppi<-length( ppis[which(ppis%in%tmp)])/length(ppis)
  ratio_corum<-length( ppis[which(ppis%in%tmp)])/length(tmp)
  
  ppi_subunits_in_corum<-paste((ppis[which(ppis%in%tmp)]),collapse = ";")
  score<-ratio_ppi*ratio_corum
out<-c(ppi_subunits_in_corum,score)
  

  } )
  
  
  allppi<-data.frame(t(allppi))
  names(allppi)<-c("ppi_subunits_in_corum","score")
  

  if (max(allppi$score)==0){
    df0<-data.frame(i,NA,length(ppis),NA,0)
    names(df0)<-c("ppi","corumid","ppi_subunits","ppi_subunits_in_corum","score")
}else{ 
  corumi<-paste(c(row.names(allppi[(which(allppi$score==max(allppi$score))),])),collapse = ";")
  ppi_subunits_in_corum<-paste(c((allppi[(which(allppi$score==max(allppi$score))),1])),collapse = "_")
  df0<-data.frame(i,corumi,length(ppis),ppi_subunits_in_corum,max(allppi$score))
    names(df0)<-c("ppi","corumid","ppi_subunits","ppi_subunits_in_corum","score")
...
write.csv(df,"out/ppi_map2corum.csv",row.names = F)

PPIselect<-read.csv("input_data/PPI_select_corr_cv2.csv")%>%
select(-1)
names(df)[1]<-"PPIs"
PPIselect2<-merge(PPIselect,df,by="PPIs",all.x=T, all.y = F)

write.csv(PPIselect2,"out/295_PPI_select.csv")
...
library(readr)
library(stringr)
selectppi0<-read.csv("out/295_PPI_select.csv")%>%
  as.data.frame()
selectppi<-selectppi0 %>%
  
  
select(PPIs,ppi_subunits,score,corumid,ppi_subunits_in_corum)%>%
mutate(anno=paste0(ppi_subunits,"-",score))
selectppi<-unique(selectppi)

allppi<-read.csv("input_data/allPPIs.csv",row.names = 1)
...
#all_name_mapped <- string_db$map(all_prot[,c(1,2)], "Protein.Group",removeUnmappedRows = F )
all_name_mapped<-read.csv("input_data/name_map2stringid.csv")

# selectppi$PPIs
for (ppi in (selectppi$PPIs)) {
  
  
anno<-c(as.matrix(selectppi[selectppi$PPIs== ppi,"anno"]))
corumid<- c(as.matrix(selectppi[selectppi$PPIs== ppi,"corumid"]))
prot_in_corum<-c(as.matrix(selectppi[selectppi$PPIs== ppi,"ppi_subunits_in_corum"]))
prot_in_corum<-gsub("_|;",";",prot_in_corum)
prot_in_corum<-c(unique(as.matrix(str_split_fixed(prot_in_corum,";",100))))
prot_in_corum<-prot_in_corum[!prot_in_corum==""]

ppi1<-c(as.matrix(allppi[row.names(allppi)==ppi,]))
ppi1<-ppi1[!ppi1==""]%>%
as.data.frame()
names(ppi1)<-"uniprot"
name_mapped <- all_name_mapped[all_name_mapped$Protein.Group %in% ppi1$uniprot,-2]

interactions <- string_db$get_interactions(name_mapped$STRING_id)

if(length(interactions$from)>0){


interactions$from <- name_mapped$Protein.Group[match(interactions$from, name_mapped$STRING_id)]
interactions$to <- name_mapped$Protein.Group[match(interactions$to, name_mapped$STRING_id)]
nodes <- interactions %>% { data.frame(gene = c(.$from, .$to)) } %>% distinct()
prot_unppi<-ppi1$uniprot[-which(ppi1$uniprot %in% nodes$gene)]
if(length(prot_unppi)==0){
  
  
}else
{
 prot_unppi<-prot_unppi

  
prot_unppi<-data.frame(prot_unppi,NA,0)
names(prot_unppi)<-names(interactions)
}

interactions<-rbind(interactions,prot_unppi)
interactions$ppi<-ppi
interactions$corumid<-corumid
interactions$anno<-anno
interactions<-unique(interactions)
interactions_table<-ppi1
interactions_table$corum<-interactions_table$uniprot %in% prot_in_corum
interactions_table$ppi<-ppi
write.csv(interactions,paste0("out/ppi_select//interactions_to_cyto/20230707/network/",ppi,"_cyto_network.csv"),quote = F,row.names = F)
write.csv(interactions_table,paste0("out/ppi_select//interactions_to_cyto/20230707/table/",ppi,"_cyto_table.csv"),quote = F,row.names = F)
...
lw.combineAllFile<-function(file=file,file.type=c("csv","txt","excel"))
{
        aa<-list.files(paste0(file))
        if(file.type=="csv"){
                df1<-read_csv(paste0(file,"/",aa[1]))
                df1<-df1[-(1:nrow(df1)),]
                for (i in 1:length(aa)) {
                        df<-read_csv(paste0(file,"/",aa[i]))
                        df1<-rbind(df1,df)
                }
        }
        
        else if(file.type=="txt")
        {df1<-read.table(paste0(file,"/",aa[1]),header = T,sep="\t")
        df1<-df1[-(1:nrow(df1)),]
        for (i in 1:length(aa)) {
                df<-read.table(paste0(file,"/",aa[i]),header = T,sep="\t")
                df1<-rbind(df1,df)
        }
        }
        
        else if(file.type=="excel")
        {df1<-read_excel(paste0(file,"/",aa[1]),sheet = 1)
        df1<-df1[-(1:nrow(df1)),]
        for (i in 1:length(aa)) {
                df<-read_excel(paste0(file,"/",aa[i]),sheet = 1)
                df1<-rbind(df1,df)
        }
        }
        df1
...
network<-lw.combineAllFile(file = "E:\\OneDrive\\OneDrive_WU\\FUDAN\\Wet_Dry_LAB\\Drylab\\ProteinComplexAnalysis\\QuantPtyrProtComplex\\out\\ppi_select\\interactions_to_cyto\\20230707/network/", file.type = "csv")

table<-lw.combineAllFile(file = "E:\\OneDrive\\OneDrive_WU\\FUDAN\\Wet_Dry_LAB\\Drylab\\ProteinComplexAnalysis\\QuantPtyrProtComplex\\out\\ppi_select\\interactions_to_cyto\\20230707/table//", file.type = "csv")

write.csv(network,paste0("out/ppi_select//interactions_to_cyto/20230707/","all_cyto_network.csv"),quote = F,row.names = F)
write.csv(table,paste0("out/ppi_select//interactions_to_cyto/20230707/","all_cyto_table.csv"),quote = F,row.names = F)



```

```{r plot all ppi network}
rm(list=ls())
library(igraph)
library(stringr)
network<-read.csv("out/ppi_select/interactions_to_cyto/20230707/all_cyto_network.csv")
table<-read.csv("out/ppi_select/interactions_to_cyto/20230707/all_cyto_table.csv")
network$Corumid<-data.frame(str_split_fixed(network$corumid,"_",2))[,1]
network$Corum<-data.frame(str_split_fixed(network$corumid,"_",2))[,2]
network$anno<-paste0(network$ppi," ",network$Corum," ","(Corum id: ",network$Corumid,")")
network$anno<-gsub("\\(Corum id: NA\\)"," ",network$anno)

pdf("out/tmp.pdf", width = 8.27, height = 15)

# Calculate the required number of rows and columns for subplots
num_plots <- length(unique(network$ppi))
num_rows <- ceiling(sqrt(num_plots))
num_cols <- ceiling(num_plots / num_rows)

par(mfrow = c(num_rows, num_cols), mar = c(1, 0.3, 0.3, 0.3))

# Loop to generate subplots
counter <- 1
for (i in unique(network$ppi)) {
    network_matrix <- network[network$ppi == i, ]
    table_matrix <- table[table$ppi == i, ]
    g <- graph.data.frame(network_matrix, directed = FALSE)
    V(g)$color <- ifelse(V(g)$name %in% table_matrix$uniprot, ifelse(table_matrix$corum == "TRUE", "#F6B782","#4EAAC7"), "gray")
    
    par(font.main = 2, cex.main = 0.3)
    
    main_title <- unique(network_matrix$anno)
    main_title_lines <- strwrap(main_title, width = 20)
    main_title_lines <- paste(main_title_lines, collapse = "\n")
    
    # Set vertex.size based on the number of nodes
    num_nodes <- vcount(g)
    if (num_nodes <= 5) {
        vertex_size <- 70
    } else if (num_nodes >= 5 & num_nodes < 8) {
        vertex_size <- 60
    } else if (num_nodes >= 8 & num_nodes < 10) {
        vertex_size <- 50
    } else if (num_nodes >= 10 & num_nodes < 13) {
        vertex_size <- 40
    } else if (num_nodes >= 13 & num_nodes < 15) {
        vertex_size <- 30
    } else if (num_nodes >= 15 & num_nodes < 20) {
        vertex_size <- 20
    } else {
        vertex_size <- 10
    }
    
    plot(g, vertex.color = V(g)$color, vertex.label = V(g)$name, layout = layout.circle, main = main_title_lines,
          vertex.label.cex = 0.8, vertex.border = "blue", vertex.size = vertex_size)
  
    counter <- counter + 1
    if (counter > num_plots) {
        break  # Exit the loop to ensure no extra subplots are generated
    }
}
